...
use crate::config;
use crate::codex_accounts::CodexQuotaSnapshot;
...
fn parse_codex_credentials_json(content: &str) -> CodexCredentials {
    ...
    let parsed_value: serde_json::Value = match serde_json::from_str(content) {
        ...
        Err(error) => {
            return (
                ...
                None,
                CredentialStatus::ParseError,
                Some(format!("Failed to parse Codex auth JSON value: {error}")),
            );
        }
    };
    ...
    let account_id = auth
        ...
        .or_else(|| extract_codex_account_id_from_claims(&parsed_value));

    let has_oauth_token = access_token.as_deref().is_some_and(|token| !token.is_empty());
    let auth_mode = auth.auth_mode.as_deref();
    let is_oauth_mode = matches!(auth_mode, Some("chatgpt") | Some("oauth") | None);

    if !has_oauth_token || !is_oauth_mode {
        return (
            None,
            None,
...
fn extract_codex_account_id_from_claims(auth_json: &serde_json::Value) -> Option<String> {
    ["id_token", "access_token"]
        ...
        .filter_map(|token_name| {
            auth_json
                ...
                .or_else(|| auth_json.get(token_name).and_then(serde_json::Value::as_str))
        })
        .find_map(|token| {
...
pub fn read_codex_credentials_from_auth_json_value(auth_json: &serde_json::Value) -> CodexCredentials {
    match serde_json::to_string(auth_json) {
        Ok(content) => parse_codex_credentials_json(&content),
        Err(error) => (
            None,
            None,
            CredentialStatus::ParseError,
            Some(format!("Failed to serialize Codex auth JSON: {error}")),
        ),
    }
}
...
pub async fn get_saved_codex_subscription_quota(
    auth_json: &serde_json::Value,
) -> Result<SubscriptionQuota, String> {
    let (token, account_id, status, message) = read_codex_credentials_from_auth_json_value(auth_json);
    ...
    match status {
        ...
        CredentialStatus::Expired => Ok(SubscriptionQuota::error(
            "codex",
            CredentialStatus::Expired,
...
pub fn codex_quota_snapshot_from_subscription(
    quota: &SubscriptionQuota,
) -> Result<CodexQuotaSnapshot, String> {
    if quota.credential_status == CredentialStatus::Expired {
        return Err(
            quota
                .credential_message
                .clone()
                .or_else(|| quota.error.clone())
                .unwrap_or_else(|| "Codex OAuth token expired".to_string()),
        );
    }
    ...
    if !quota.success {
        return Err(
            quota
                .error
                .clone()
                .unwrap_or_else(|| "Failed to refresh Codex quota".to_string()),
        );
    }
        ...
        match tier.name.as_str() {
            "five_hour" => {
                snapshot.five_hour_percent =
                    Some(codex_remaining_percent_from_used_percent(tier.utilization));
                snapshot.five_hour_reset_at = tier
                    .resets_at
                    .as_deref()
...
fn codex_remaining_percent_from_used_percent(used_percent: f64) -> f64 {
...
mod tests {
    use super::{
        codex_quota_snapshot_from_subscription, CredentialStatus, QuotaTier, SubscriptionQuota,
    };

    fn sample_codex_subscription(tiers: Vec<QuotaTier>) -> SubscriptionQuota {
        SubscriptionQuota {
            tool: "codex".to_string(),
            credential_status: CredentialStatus::Valid,
            credential_message: None,
    ...
    #[test]
    fn codex_quota_snapshot_converts_used_percent_to_remaining_percent() {
        let quota = sample_codex_subscription(vec![
            QuotaTier {
                name: "five_hour".to_string(),
                utilization: 9.0,
                resets_at: Some("2026-04-06T16:29:00Z".to_string()),
            },
            QuotaTier {
                name: "seven_day".to_string(),
                utilization: 26.0,
                resets_at: Some("2026-04-13T00:00:00Z".to_string()),
            },
        ]);

        let snapshot =
            codex_quota_snapshot_from_subscription(&quota).expect("quota snapshot should build");

        assert_eq!(snapshot.five_hour_percent, Some(91.0));
        assert_eq!(snapshot.week_percent, Some(74.0));
    }

    #[test]
    fn codex_quota_snapshot_clamps_remaining_percent_to_valid_range() {
        let quota = sample_codex_subscription(vec![
            QuotaTier {
                name: "five_hour".to_string(),
                utilization: -5.0,
        ...
        assert_eq!(snapshot.five_hour_percent, Some(100.0));
        assert_eq!(snapshot.week_percent, Some(0.0));
    }
}
...
fn read_gemini_oauth_client() -> Option<(String, String)> {
...
async fn refresh_gemini_token(refresh_token: &str) -> Option<String> {
    let (client_id, client_secret) = read_gemini_oauth_client()?;
    let client = crate::proxy::http_client::get();
