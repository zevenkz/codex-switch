...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.codex-switch/skills/`
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（~/.codex-switch/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.codex-switch/skill-backups/）
    ...
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "codex-switch".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "codex-switch".to_string()));
