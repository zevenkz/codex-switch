...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/codex-switch.log）
            {
                ...
                let log_file_path = log_dir.join("codex-switch.log");
            ...
            crate::codex_accounts::watcher::spawn_codex_auth_watcher(app.handle().clone());
            ...
            // 静默启动：开发模式始终显示主窗口，便于本地调试真实数据流。
            let settings = crate::settings::get_settings();
            let should_show_window = cfg!(debug_assertions) || !settings.silent_startup;
            if let Some(window) = app.get_webview_window("main") {
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_active_codex_account,
            commands::start_codex_account_oauth,
            commands::cancel_codex_account_oauth,
            commands::complete_codex_account_oauth,
            commands::switch_codex_account,
            commands::delete_codex_account,
            commands::refresh_codex_account_quota,
            commands::refresh_all_codex_account_quotas,
